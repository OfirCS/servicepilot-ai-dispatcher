...
function buildOutreach(name: string, config: SegmentConfig, city: string) {
  ...
  return (
    `Hi ${name} — this is ${COMPANY.ownerName} at ${COMPANY.name}. ` +
...
const USER_AGENT = 'ServicePilot/1.0 (lead-finder; contact: demo@servicepilot.app)'
...
function elementToProspect(element: OsmElement): Prospect | null {
...
export async function findProspects(
  area: string,
  radiusKm = 6,
): Promise<{ prospects: Prospect[]; area: string; center?: { lat: number; lon: number }; source: string }> {
  const query = area.trim() || 'North York, Toronto'
  ...
  if (!geo?.length) {
    return { prospects: [], area: query, source: 'openstreetmap' }
  }
...
export function liveSourceEnabled() {
  // Allows an operator to force the demo/sample path off in production.
  return getEnv('SERVICEPILOT_LEAD_SOURCE') !== 'off'
}
